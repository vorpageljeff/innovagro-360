...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const current = groups.flatMap((group) => group.items).find((item) => pathname.startsWith(item.href))?.label ?? "Workspace";
  return <div className="shell"><aside className="sidebar"><div className="brand"><b className="brand-mark">360</b><span>InnovAgro</span></div><nav>{groups.map((group) => <div key={group.label || "home"}>{group.label && <div className="nav-label">{group.label}</div>}{group.items.map((item) => { const Icon = item.icon; const active = pathname === item.href || pathname.startsWith(`${item.href}/`); return <Link className={`nav-item ${active ? "active" : ""}`} href={item.href} key={item.href} aria-current={active ? "page" : undefined}><Icon /><span>{item.label}</span></Link>; })}</div>)}</nav><Link className="sidebar-foot" href="/configuracoes"><Settings size={16} /><span>Configurações</span></Link></aside><main className="main"><header className="topbar"><span className="muted">Workspace / {current}</span><div className="top-actions"><button className="btn"><Search size={15} /> Buscar <kbd>⌘ K</kbd></button><button className="btn primary"><Plus size={15} /> Criar</button><button className="btn" aria-label="Notificações"><Bell size={16} /></button></div></header>{children}</main></div>;
}
